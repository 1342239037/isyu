...
def calcRegLoss(model):
    """
    Calculate the regularization loss by summing the L2 norm of model parameters.通过对模型参数的 L2 范数求和来计算正则化损失。
...
def contrastLoss(embeds1, embeds2, nodes, temp):
    """
    Calculate the contrastive loss for embeddings.对比学习损失
    ...
    """
    embeds1 = F.normalize(embeds1 + 1e-8, p=2)#P=2代表进行正则2
